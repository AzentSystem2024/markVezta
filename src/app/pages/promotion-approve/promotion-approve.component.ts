...
@Component({
  selector: 'app-promotion-approve',
  templateUrl: './promotion-approve.component.html',
  styleUrls: ['./promotion-approve.component.scss'],
})
export class PromotionApproveComponent {
  ...
  onSelectionChangedforNewItem(event: any) {
    ...
    this.selectedRowsOfPopup = this.filteredItemStoresListRemaining.filter(
      (item: any) => this.selectedRowKeysNew.includes(item.ID), // Replace `ID` with your keyExpr field
    );
    this.isRowsSelected = this.selectedRowsOfPopup.length > 0;
    console.log(
      this.selectedRowsOfPopup,
      'Manually fetched selected rows data',
    );
      ...
      if (selectedSchema) {
        if (selectedSchema.REMARKS !== '4') {
          ...
          const additionalSchemaItems = this.filteredItems.map((item) => ({
            ...
            PROMOTION_NAME:
              selectedSchema.DESCRIPTION ||
              (Array.isArray(item.PROMOTION_NAME)
...
@NgModule({
  ...
  providers: [],
  exports: [PromotionApproveComponent],
  declarations: [PromotionApproveComponent],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class PromotionApproveModule {}
