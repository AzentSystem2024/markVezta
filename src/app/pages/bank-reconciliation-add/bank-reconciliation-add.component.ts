...
  }
  ...
  calculateTotals() {
  this.totalDebit = this.BankReconciliationdatasource.reduce((sum, item) => sum + (Number(item.DR_AMOUNT) || 0), 0);
  this.totalCredit = this.BankReconciliationdatasource.reduce((sum, item) => sum + (Number(item.CR_AMOUNT) || 0), 0);
  ...
  const selectedDebitSum = this.selectedRows.reduce((s: number, r: any) => s + (Number(r.DR_AMOUNT) || 0), 0);
  const selectedCreditSum = this.selectedRows.reduce((s: number, r: any) => s + (Number(r.CR_AMOUNT) || 0), 0);
