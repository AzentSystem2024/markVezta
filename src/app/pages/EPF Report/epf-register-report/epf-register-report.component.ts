  ...
  getEPFRegister(): void {
    ...
    this.dataService.getEPFRegister(payload).subscribe((response: any) => {
      if (!response?.EPFDetails || response.EPFDetails.length === 0) {
        notify(
          {
            message: 'No EPF details found for selected month',
      ...
      doc.setFontSize(12);
      doc.text(
        `EPF Day Book for the month of ${this.selectedMonth}`,
        doc.internal.pageSize.getWidth() / 2,
        50,
        { align: 'center' },
      );
      ...
      const head = [
        [
          ...
          {
            content: 'Employer Share',
            colSpan: 2,
            styles: { halign: 'center' as const },
          },
        ],
        [{ content: 'A/C.01' }, { content: 'A/C.10' }],
      ];
