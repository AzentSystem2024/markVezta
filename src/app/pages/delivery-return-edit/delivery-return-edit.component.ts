...
export class DeliveryReturnEditComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (
      changes['SelectDeliveryReturnData'] &&
      changes['SelectDeliveryReturnData'].currentValue
    ) {
  ...
  onCellValueChanged(e: any) {
    ...
    if (e.dataField === 'QUANTITY') {
      ...
      if (qtyReturned > qtyAvailable) {
        notify(
          'Qty Returned cannot be greater than Qty Available',
          'error',
          2000,
        );
  ...
  editDeliveryreturn() {
    ...
    if (invalidRows.length > 0) {
      notify(
        'One or more rows have Qty Returned greater than Qty Available',
        'error',
        3000,
      );
      return; // stop saving
