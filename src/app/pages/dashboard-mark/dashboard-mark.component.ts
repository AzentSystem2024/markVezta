...
@Component({
  selector: 'app-dashboard-mark',
  templateUrl: './dashboard-mark.component.html',
  styleUrls: ['./dashboard-mark.component.scss'],
})
export class DashboardMarkComponent {
  dateRanges = [
  ...
  getDashboardData() {
    ...
    console.log(payload);
    this.service.dashboardMarkData(payload).subscribe(
      (res: any) => {
        clearTimeout(timeoutId); // stop timeout

        this.loadingVisible = false;
        this.lastMonthSalesList = res.data.LastMonthSales.map((item: any) => ({
          ITEM_NAME: item.ITEM_NAME,
          NET_AMOUNT: item.NET_AMOUNT,
        }));
        // this.TopMovingArticles_list = res.data.
        const article_list = res.data.TopMovingArticles;
        console.log(article_list, '==========item_list===========');
        const maxQty = article_list.reduce((max: number, item: any) => {
          return item.TOTAL_QTY > max ? item.TOTAL_QTY : max;
        }, 0);
        console.log(maxQty, '==========maxQty===========');

        this.TopMovingArticles_list = article_list.map((item: any) => ({
          ART_NO: item.ART_NO,
        ...
        const groupedRegions = Object.values(
          res.data.RegionWiseSales.reduce((acc: any, item: any) => {
            ...
            acc[item.STATE_NAME].TOTAL_INVOICES += item.TOTAL_INVOICES;
            acc[item.STATE_NAME].GROSS_AMOUNT += item.GROSS_AMOUNT;
            acc[item.STATE_NAME].TAX_AMOUNT += item.TAX_AMOUNT;
            acc[item.STATE_NAME].TOTAL_SALES += item.TOTAL_SALES;
  ...
  customizeRegionTooltip = (arg: any) => {
    ...
    return {
      text:
        ...
        '\n' +
        'Gross Amount : ' +
        this.formatNumber(data.GROSS_AMOUNT) +
        '\n' +
        'Tax Amount : ' +
        this.formatNumber(data.TAX_AMOUNT) +
        '\n' +
        'Total Sales : ' +
  ...
  customizeArticleTooltip(arg: any) {
    const data = arg.point.data;

    return {
      text: `
      Article : ${data.ARTICLE_NAME}
      Article No : ${data.ART_NO}
      Total Qty : ${this.formatNumber(data.TOTAL_QTY)}
      Total Transactions : ${this.formatNumber(data.TOTAL_TRANSACTIONS)}
    `,
    };
  }
  ...
  customizeChartTooltip(arg: any) {
    ...
    return {
      text: `
      Article  : ${arg.argumentText}
      Qty  : ${new Intl.NumberFormat('en-IN').format(arg.value)}
      Article No : ${arg.seriesName}
    `,
    };
  }
...
${this.formatAmount(arg.value)}`;
  ...
  customizePoint = (pointInfo: any) => {
    ...
    const index = this.TopMovingArticles_list.findIndex(
      (x: any) => x.ARTICLE_NAME === pointInfo.argument,
    );
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [DashboardMarkComponent],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class DashboardMarkModule {}
