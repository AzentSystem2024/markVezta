...
@Component({
  ...
  styleUrls: ['./consignment-summary.component.scss'],
})
export class ConsignmentSummaryComponent {
  ...
  load_JournalBook_data() {
    ...
    this.JournalBookDataSource = new DataSource({
      load: () =>
        new Promise((resolve) => {
          this.dataService.ConsignmentSummary_Report(payload).subscribe({
            next: (res: any) => {
              const list = res?.data || [];
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [ConsignmentSummaryComponent],
})
export class ConsignmentSummaryModule {}
