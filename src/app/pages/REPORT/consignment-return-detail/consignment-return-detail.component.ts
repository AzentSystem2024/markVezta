...
@Component({
  selector: 'app-consignment-return-detail',
  templateUrl: './consignment-return-detail.component.html',
  styleUrls: ['./consignment-return-detail.component.scss'],
})
export class ConsignmentReturnDetailComponent {
  ...
  load_JournalBook_data() {
    const payload = {
      ...
      STATUS_ID: '',
      CUSTOMER_ID:
        this.selectedCustomer != null ? String(this.selectedCustomer) : 0,
      BRAND_ID: 0,
    };
    ...
    this.JournalBookDataSource = new DataSource({
      load: () =>
        new Promise((resolve) => {
          this.dataService.ConsignmentReturn_Details_Report(payload).subscribe({
            next: (res: any) => {
              const list = res?.data || [];
...
@NgModule({
  ...
  providers: [],
  exports: [],
  declarations: [ConsignmentReturnDetailComponent],
})
export class ConsignmentReturnDetailModule {}
