    ...
    try {
      ...
      if (!this.arraysMatch(excelHeaders, expectedHeaders)) {
        ...
        notify(
          {
            message:
              'Excel template columns do not match required format.',
            position: { at: 'top right', my: 'top right' },
          },
          'error',
          4000
    ...
    this.datasource = new DataSource({
      load: () =>
        new Promise((resolve, reject) => {
          this.service.getImportChartOfAccountsLog(this.selectedCompanyId).subscribe({
            next: (data: any[]) => {
  ...
  this.service
    ...
    .subscribe({
      ...
      error: () => {
        ...
        notify(
          'Failed to load imported accounts',
          'error',
          3000
        );
      }
...
saveImportedData() {
  ...
  this.service.saveimportedAccounts(payload)
    .subscribe({
      next: () => {
