...
export class RefundEditComponent {
  ...
  transactionTypes = [
    { text: 'Deposit', value: 106 },
    { text: 'Refund', value: 107 }
  ];
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['receiprtFormData'] && this.receiprtFormData) {
      ...
      this.customerType = firstReceipt.CUSTOMER_ID ? 'Dealer' : 'Unit';
    }
  }
  ...
  commitReceipt(): void {
    ...
    this.receiprtFormData.NET_AMOUNT = this.receiprtFormData.NET_AMOUNT;
    ...
    const commitPayload = {
      TRANS_ID: this.receiprtFormData.TRANS_ID,
      TRANS_TYPE: this.receiprtFormData.TRANS_TYPE,
      REC_NO: this.receiprtFormData.REC_NO,
      REC_DATE: this.receiprtFormData.REC_DATE,

      COMPANY_ID: this.selectedCompanyId,
      STORE_ID: this.selectedstoreId,
      FIN_ID: this.receiprtFormData.FIN_ID,

      REF_NO: this.receiprtFormData.REF_NO,

      CHEQUE_NO: this.receiprtFormData.CHEQUE_NO,
      CHEQUE_DATE: this.receiprtFormData.CHEQUE_DATE,
      BANK_NAME: this.receiprtFormData.BANK_NAME,

      PDC_ID: this.receiprtFormData.PDC_ID || 0,

      CUSTOMER_ID: this.selectedDistributorId,

      PARTY_NAME: this.receiprtFormData.PARTY_NAME,

      NARRATION: this.receiprtFormData.NARRATION,

      CREATE_USER_ID: Number(sessionStorage.getItem('UserId')),

      PAY_TYPE_ID: this.receiprtFormData.PAY_TYPE_ID,
      PAY_HEAD_ID: this.receiprtFormData.PAY_HEAD_ID,

      NET_AMOUNT: this.receiprtFormData.NET_AMOUNT,

      IS_APPROVED: true
    };

    this.dataService.commitRefundDeposit(commitPayload).subscribe({
      next: () => {
        this.isSaving = false;
  ...
  UpdateReceipt() {
    ...
    // Call API
    this.dataService.updateRefundDeposit(payload).subscribe({
      next: (response: any) => {
        this.isSaving = false;
        if (response.flag == 1) {
  ...
  VerifyReceipt() {
    ...
    this.dataService.verifyRefundDeposit(payload).subscribe({
      next: (response: any) => {
        this.isSaving = false;
...
@NgModule({
  ...
  exports: [RefundEditComponent],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class RefundEditModule { }