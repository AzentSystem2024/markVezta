...
import { RefundAddModule } from '../refund-add/refund-add.component';
import { RefundEditModule } from '../refund-edit/refund-edit.component';
...
export class RefundComponent {
  ...
  get popupTitle(): string {
    switch (this.formMode) {
      case 'edit':
        return 'Update Deposit/Refund';

      case 'verify':
        return 'Verify Deposit/Refund';

      case 'approve':
        return 'Approve Deposit/Refund';

      case 'view':
        return 'View Deposit/Refund';
  ...
  transactionTypes = [
    { ID: 106, DESCRIPTION: 'Deposit' },
    { ID: 107, DESCRIPTION: 'Refund' }
  ];

  selectedTransType = 106;
  ...
  onTransTypeChanged(e: any) {
    this.selectedTransType = e.value;
    this.getCustomerReceipts();
  }
  ...
  getCustomerReceipts(dateRange: string = this.selectedDateRange) {
    ...
    const payload = {
      ...
      TRANS_TYPE: this.selectedTransType === 0 ? null : this.selectedTransType,
    };

    this.ReceiptDataSource = new DataSource({
      load: () =>
        new Promise((resolve) => {
          this.dataService.getRefundDeposit(payload).subscribe({
            next: (response: any) => {
  ...
  onVerifyInvoice(e: any) {
    ...
    const transType = e.row.data.TRANS_TYPE
    const payload = {
      TRANS_ID: receiptId,
      TRANS_TYPE: transType
    }
    this.dataService
      .selectRefundDeposit(payload)
  ...
  onEditReceiptNote(event: any) {
    ...
    const transType = event.data.TRANS_TYPE;
    const payload = {
      TRANS_ID: receiptId,
      TRANS_TYPE: transType
    }
  ...
  onDeleteReceiptNote(event: any) {
    ...
    this.dataService.deleteRefundDeposit(payload).subscribe(
      (response: any) => {
        if (response) {
          notify(
            {
