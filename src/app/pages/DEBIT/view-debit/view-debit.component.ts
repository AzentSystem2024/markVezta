...
ngOnChanges(changes: SimpleChanges): void {
  if (changes['debitFormData'] && this.debitFormData?.length) {
    ...
    if (companyState === customerState) {
      console.log("Same State → CGST + SGST");
      ...
      console.log("Different State → IGST");
    ...
    // -----------------------------
    // ⭐ ALWAYS APPLY ROUND OFF HERE
    // -----------------------------
    setTimeout(() => {
      const rawNet = Number(this.debitFormData[0].NET_AMOUNT) || 0;

      if (this.debitFormData[0].ROUND_OFF === true) {
        this.debitFormData[0].NET_AMOUNT = Math.round(rawNet);
      } else {
        this.debitFormData[0].NET_AMOUNT = rawNet;
      }

      console.log("FINAL NET AMOUNT:", this.debitFormData[0].NET_AMOUNT);
    });
...
get_pdf(data: any): SafeResourceUrl {
  ...
  doc.text(`State : ${data.SUPP_STATE_NAME}, Code : 32`, blueX + 3, blueY + 33);
  doc.text(`E-Mail : ${data.EMAIL}`, blueX + 3, blueY + 38);
  ...
  doc.text(`State : ${data.SUPP_STATE_NAME}, Code : 32`, rightX, rightY + gap * 6);
  ...
  doc.text(`State : ${data.SUPP_STATE_NAME}, Code : 32`, rightX, buyerY + gap * 6);
  ...
  // ======================================================
  const columns = [
    "Ledger Code", "Ledger Name", "Particular",
    "Amount", "GST Amount", "CGST", "SGST", "HSN Code", "Total"
  ];
  ...
  const footerRow = ["", "", "", "", "", "", "", "Total", data.NET_AMOUNT.toFixed(2)];
...
const gstPerc = Number(data.NOTE_DETAIL[0].CGST || 0) + Number(data.NOTE_DETAIL[0].SGST || 0);
