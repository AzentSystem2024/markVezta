...
import {
  ...
  SalesmanFormModule,
} from 'src/app/components/library/salesman-form/salesman-form.component';
...
export class SalesmanListComponent implements OnInit {
  ...
  showSalesman() {
    this.dataservice.getSalesmanData().subscribe((response) => {
  ...
  onClickSaveSalesman() {
    const {
      ...
      IQAMA_NO,
      INCENTIVE_PERCENT,
    } = this.salesmanComponent.getNewSalesmanData();
  ...
  onRowUpdating(event) {
    ...
    this.dataservice.updateSalesman(combinedData).subscribe((data: any) => {
      if (data) {
        notify(
          {
