...
export class PrePaymentAddComponent {
  ...
  savePrePayment() {
    ...
    if (this.PrePaymentFormData.IS_APPROVED) {
      ...
      return; // stop the normal execution here
    }

    // If NOT approved → call insert API directly
    this.callInsertPrePayment(payload);
  ...
  callInsertPrePayment(payload: any) {
    ...
    this.dataservice.Insert_PrePayment(payload).subscribe(
      (res: any) => {
        ...
        if (res.Message === 'Success') {
          notify({
            message: this.PrePaymentFormData.IS_APPROVED
              ? 'PrePayment Approved Successfully'
              : 'PrePayment Saved Successfully',
            type: 'success',
            position: { at: 'top right', my: 'top right' },
            displayTime: 500,
          });
