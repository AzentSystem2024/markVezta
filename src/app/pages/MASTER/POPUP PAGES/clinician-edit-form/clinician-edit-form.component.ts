...
export class ClinicianEditFormComponent implements OnChanges {
  ...
  get_DropDown_Data() {
    ...
    this.datasrvce.Get_GropDown('SPECIALITY').subscribe((response: any) => {
      this.specialityDatasource = response;
    });

    this.datasrvce.Get_GropDown('CLINICIANMAJOR').subscribe((response: any) => {
    ...
    this.datasrvce.Get_GropDown('GENDER').subscribe((response: any) => {
    ...
    this.datasrvce.getDropdownData(dept_payload).subscribe((data) => {
  ...
  getCliniciansData() {
    this.datasrvce.get_Clinian_Table_Data().subscribe((res: any) => {
