...
>
  ...
  Add_Designation() {
    ...
    const isDuplicate = this.Designation.some((data: any) => {
      return (
        data.DESG_NAME?.toLowerCase().trim() ===
          DESG_NAME?.toLowerCase().trim() ||
    ...
    if (CODE && DESG_NAME) {
      this.dataservice
        .Insert_Designation_Api(CODE, DESG_NAME, IS_INACTIVE)
        .subscribe((response) => {
          notify(
            {
  ...
  Edit_Designation() {
    ...
    const isDuplicate = this.Designation?.some((item: any) => {
      ...
      return (
        ...
        (item.DESG_NAME?.trim().toLowerCase() || '') ===
          (DESG_NAME?.trim().toLowerCase() || '')
      );
    });
    ...
    if (CODE && DESG_NAME) {
      this.dataservice
        .Update_Designation_Api(ID, CODE, DESG_NAME, IS_INACTIVE)
