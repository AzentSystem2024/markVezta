...
@Component({
  selector: 'app-purchase-order-approve-form',
  templateUrl: './purchase-order-approve-form.component.html',
  styleUrls: ['./purchase-order-approve-form.component.scss']
})
export class PurchaseOrderApproveFormComponent implements OnInit,OnChanges {

  @Output() netApproveAmountChange = new EventEmitter<any>();
  @Output() netApproveSupplierAmountChange = new EventEmitter<any>();
  @Output() netApproveQuantityChange = new EventEmitter<number>();
  ...
  saveSelectedData() {
    this.savedItems = this.selectedItems.map((item, index) => {
      // Check if supplier currency differs from local currency
      const useSupplierPrice = this.SupplierCurrencyCode !== this.localCurrencyCode;
      console.log(useSupplierPrice,"useSupplierPrice")

      // If they differ, set SUPP_PRICE to the value of PURCH_PRICE
      const supplierPrice = useSupplierPrice ? item.PURCH_PRICE : 0;
      console.log(supplierPrice,"supplier price")

      return {
        ...item,
        slNo: index + 1,  // Add serial number starting from 1
        SUPP_PRICE: supplierPrice, // Set SUPP_PRICE based on currency check
        PURCH_PRICE: useSupplierPrice ? (supplierPrice / this.currencyExchangeRate).toFixed(2) : item.PURCH_PRICE,
    ...
    console.log(this.savedItems,"saved items")
  ...
  calculateTotalQuantity() {
    ...
    this.netApproveQuantityChange.emit(this.totalQuantity);
}
...
calculateTotalIncludingTax() {
  ...
  this.netApproveAmountChange.emit(amountToEmitInLocalCurrency); // Emit NET_AMOUNT with the currency

  // Emit SUPP_NET_AMOUNT with the supplier currency symbol
  this.netApproveSupplierAmountChange.emit(amountToEmitInSupplierCurrency); // Emit SUPP_NET_AMOUNT with the supplier currency symbol
