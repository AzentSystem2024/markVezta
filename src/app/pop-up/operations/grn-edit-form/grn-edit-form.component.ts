...
export class GrnEditFormComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes.formdata && changes.formdata.currentValue) {
      ...
      console.log(this.newGrnData.GRN_Item_Cost, 'ITEMCOSTTTTTTTTTT');
  ...
  updateCell(event: any) {
    ...
    if ('QUANTITY' in updatedData) {
      ...
      this.formattedLocalNetAmount = `${this.localCurrencySymbol || ''} ${
  ...
  onCostDropdownValueChanged(e: any) {
    ...
    if (selectedCost) {
      ...
      this.costData.TOTAL = selectedCost.IS_FIXED_AMOUNT
        ? this.costData.RATE
        : (Number(this.newGrnData.NET_AMOUNT * this.costData.RATE) / 100) | 0;
    } else {
