...
export class ImportItemTemplateEditFormComponent implements OnChanges,OnInit {
  ...
  editform: ImportItemTemplateEditFormComponent;
  ...
  onReorder = (e: Parameters<DxDataGridTypes.RowDragging['onReorder']>[0]) => {
    ...
    const toIndex = this.TemplateColumnsData.findIndex(
      (item) => item.COLUMN_ID === visibleRows[e.toIndex].data.COLUMN_ID,
    );
    const fromIndex = this.TemplateColumnsData.findIndex(
      (item) => item.COLUMN_ID === e.itemData.COLUMN_ID,
    );
  ...
  onSelectionChanged(e: any) {
    ...
    const remainingColumns = this.TemplateColumnsData.filter(
      (col) => !selectedColumnsSet.has(col.COLUMN_TITLE),
    );
    this.TemplateColumnsData = [
      ...this.TemplateColumnsData.filter((col) =>
        selectedColumnsSet.has(col.COLUMN_TITLE),
      ),
      ...remainingColumns,
    ];

    this.formModel.import_entry = e.selectedRowsData.map((id) => ({
  ...
  onExportClick() {
    const selectedColumns = this.TemplateColumnsData.filter((col) =>
      this.selectedRows.includes(col.COLUMN_TITLE),
    );
