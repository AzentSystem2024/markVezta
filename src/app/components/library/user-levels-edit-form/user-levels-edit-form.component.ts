...
export class UserLevelsEditFormComponent implements OnInit, OnChanges {
  ...
  onTabClick(event: any): void {
    ...
    if (this.selectedRows[this.selectedTab]) {
      ...
      this.selectedTabData = this.selectedRows[this.selectedTab].map(
        (item: any) => ({
          ID: item.ID,
    ...
    unselectedTabData = this.MenuDatasource.filter(
      (item: any) =>
        item.MAIN_MODULE_ID === newTabIndex &&
        !this.selectedTabData.some((selected) => selected.ID === item.ID),
    ).map((item: any) => ({
      ID: item.ID,
      MODULE_NAME: item.MODULE_NAME,
      canAdd: item.canAdd || false,
    ...
    this.selectedRows[this.selectedTab] = this.selectedTabData.filter(
      (item) => {
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['formdata'] && this.formdata) {
      ...
      const selectedMenuIds = this.formdata.rights.map(
        (menu: any) => menu.MODULE_ID,
      );

      const uniqueTabIds = [
        ...new Set(
          this.MenuDatasource.filter((menu: any) => menu.MAIN_MODULE_ID !== 0) // Exclude 0
            .map((menu: any) => menu.MAIN_MODULE_ID),
        ),
      ];

      this.selectedRows[this.selectedTab] = this.MenuDatasource.filter(
        (menu: any) => selectedMenuIds.includes(menu.ID),
      ).map((menu: any) => menu.ID);

      // Loop through all unique tabs and assign selected rows for each tab
      uniqueTabIds.forEach((tabId: number) => {
        // Cast tabId to number if applicable
        this.selectedRows[tabId] = this.selectedRows[tabId] || [];
        this.selectedRows[tabId] = this.MenuDatasource.filter(
          (menu: any) =>
            menu.MAIN_MODULE_ID === tabId && selectedMenuIds.includes(menu.ID),
        ).map((menu: any) => {
          // Find the corresponding rights for the menu ID
          const right = this.formdata.rights.find(
            (r: any) => r.MODULE_ID === menu.ID,
          );
  ...
  updateTabData(tabIndex: number): void {
    this.selectedTabData = this.MenuDatasource.filter(
      (item: any) => item.MAIN_MODULE_ID === tabIndex,
    ).map((item: any) => {
