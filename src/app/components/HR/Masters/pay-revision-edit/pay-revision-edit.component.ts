...
@Component({
  selector: 'app-pay-revision-edit',
  templateUrl: './pay-revision-edit.component.html',
  styleUrls: ['./pay-revision-edit.component.scss'],
})
export class PayRevisionEditComponent {
  @Output() popupClosed = new EventEmitter<void>();
  editPayRevisionPopupOpened = false;
  addPayRevisionPopupOpened = false;
  ...
  ngOnChanges(changes: SimpleChanges) {
    if (changes['revisionData'] && changes['revisionData'].currentValue) {
      ...
      if (changes['revisionData'].currentValue.REV_DETAIL) {
        this.revisionGridData = this.revisionFormData.REV_DETAIL.map(
          (item) => ({
            code: item.HEAD_ID,
  ...
  onUpdate(): void {
    ...
    this.revisionFormData.REV_DETAIL = this.revisionFormData.REV_DETAIL.map(
      (item, index) => ({
        ...item,
        REVISED_AMOUNT:
          this.revisionGridData[index]?.newAmount ?? item.REVISED_AMOUNT,
      }),
    );

    // Make the API call
    this.dataService.updateSalaryRevision(this.revisionFormData).subscribe(
      (response) => {
        if (response) {
          notify(
            {
...
@NgModule({
  ...
  providers: [],
  declarations: [PayRevisionEditComponent],
  exports: [PayRevisionEditComponent],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class PayRevisionEditModule {}
