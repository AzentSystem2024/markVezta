...
@Component({
  selector: 'app-payroll-approve',
  templateUrl: './payroll-approve.component.html',
  styleUrls: ['./payroll-approve.component.scss'],
})
export class PayrollApproveComponent {
  ...
  onEditorPreparing(e: any) {
    if (e.dataField === 'DEDUCT' && e.row?.data?.HEAD_TYPE === 1) {
      ...
      (e.dataField === 'GROSS' && e.row?.data?.HEAD_TYPE === 2) ||
      e.row?.data?.HEAD_TYPE === 3
    ) {
      e.editorOptions = e.editorOptions || {};
      e.editorOptions.readOnly = true;
    } else {
  ...
  approve() {
    ...
    this.dataService.approvePayroll(payRollData).subscribe((response: any) => {
      if (response.flag == '1') {
        notify(
          {
...
@NgModule({
  ...
  providers: [],
  declarations: [PayrollApproveComponent],
  exports: [PayrollApproveComponent],
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class PayrollApproveModule {}
